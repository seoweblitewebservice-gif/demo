...
export default async function LocalizedHome({params}:Props){ const {locale}=await params; if(!isLocale(locale))notFound(); return <div className="doc">
  ...
  <section className="mx-auto max-w-3xl py-10"><h2 className="sect-h">Map tools</h2><p className="mt-3">Browse the full MapBench tool directory. Interactive calculations run in your browser and results can be shared by URL.</p>{CATEGORIES.map(cat=><div key={cat.id} className="mt-8"><h3 className="font-sans text-[11px] font-extrabold uppercase tracking-[0.14em] text-mute">{cat.label}</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{TOOLS.filter(t=>t.category===cat.id && isToolLocalized(locale,t.slug)).slice(0,12).map(t=><Link key={t.slug} href={`/${locale}/tools/${t.slug}`} className="card p-3"><div className="font-bold">{t.name}</div><div className="mt-1 text-xs text-mute">{t.short}</div></Link>)}</div></div>)}</section>
